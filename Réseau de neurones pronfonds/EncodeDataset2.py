...
# Entraîner le modèle
model.fit(train_sequences, train_labels_one_hot, epochs=10, batch_size=128, validation_data=(val_sequences, val_labels_one_hot))

# Évaluation du modèle
loss, accuracy = model.evaluate(val_sequences, val_labels_one_hot)
print(f'Validation Loss: {loss:.4f}, Validation Accuracy: {accuracy:.4f}')

model.save('ModelEncodeDataset.h5')
...
result_df.to_csv('predictions.txt', sep=' ', header=False, index=False)
